...
(function(){
	...
	L.HexbinLayer = L.Class.extend({
		...
		/*
		 * Getter/setter for the scale range
		 */
		scaleRange: function(min, max) {
			if(undefined === min || undefined === max){
				return {
					min: this.options.valueFloor,
					max: this.options.valueCeil
				};
			}

			this.options.valueFloor = min;
			this.options.valueCeil = max;
			this._redraw();
			return this;
		},
		...
		_createHexagons : function(g, data) {
			...
			join.transition().duration(that.options.duration)
				...
				.attr('stroke', that.options.strokeColor ? that.options.strokeColor : 'none')
				.attr('stroke-width', function(d){
					return that.options.value(d) && that.options.strokeWidth ? that.options.strokeWidth : 0;
				})
				.attr('stroke-opacity', that.options.strokeOpacity || that.options.opacity);
